...
class CacheController {
    ...
    async clearCache(req, res) {
        try {
            ...
            const cleared = await this.cacheService.clearAllCache();
    ...
    async getCacheHealth(req, res) {
        try {
            ...
            const overallHealth = {
                healthy: cacheStatus.redis?.connected && schedulerHealth.healthy,
                components: {
                    redis: {
                        healthy: cacheStatus.redis?.connected || false,
                        status: cacheStatus.redis?.connected ? 'connected' : 'disconnected'
                    },
                    scheduler: {
                    ...
                    dataFreshness: {
                        healthy: !cacheStatus.dataAge?.isStale,
                        ageHours: cacheStatus.dataAge?.hours,
                        maxAgeHours: cacheStatus.dataAge?.maxAgeHours,
                        stale: cacheStatus.dataAge?.isStale
                    }
                },
                timestamp: new Date().toISOString()
            };
