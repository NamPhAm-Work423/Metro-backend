const PayPalWebhookService = require('../services/paypalWebhook.service');
const { logger } = require('../config/logger');
const { defaultRateLimiter } = require('../middlewares/rateLimiter');

/**
 * PayPal Webhook Controller
 * Handles HTTP requests for PayPal webhooks
 * Implements Single Responsibility Principle (SRP)
 */
class PayPalController {
    constructor() {
        this.webhookService = new PayPalWebhookService();
    }
    ...
    async handleWebhook(req, res) {
        ...
        const requestId = req.headers['x-request-id'] || `paypal-${Date.now()}`;
        ...
        try {
            ...
            if (!req.body || !req.body.id || !req.body.event_type) {
                logger.warn('Invalid PayPal webhook request body', {
                    requestId,
                    hasBody: !!req.body,
                    hasId: !!req.body?.id,
            ...
            if (!isValidStructure) {
                logger.warn('PayPal webhook structure validation failed', {
                    requestId,
                    eventType: req.body.event_type,
                    webhookId: req.body.id
                });
            ...
            logger.info('PayPal webhook processing completed', {
                requestId,
                webhookId: req.body.id,
                eventType: req.body.event_type,
            ...
            logger.error('PayPal webhook controller error', {
                requestId,
                error: error.message,
                stack: error.stack,
                eventType: req.body?.event_type,
    ...
    async getStatistics(req, res) {
        try {
            ...
            logger.info('PayPal webhook statistics requested', {
            ...
            logger.error('Failed to get PayPal webhook statistics', {
                error: error.message,
                stack: error.stack,
                ip: req.ip
            });
    ...
    async retryFailedWebhooks(req, res) {
        try {
            ...
            logger.info('PayPal webhook retry requested', {
                limit: retryLimit,
                ip: req.ip,
                userAgent: req.headers['user-agent']
            });
            ...
            logger.info('PayPal webhook retry completed', {
                summary,
                ip: req.ip
            });
            ...
            logger.error('Failed to retry PayPal webhooks', {
                error: error.message,
                stack: error.stack,
                ip: req.ip
            });
    ...
    /**
     * Health check for PayPal webhook endpoint
     * @param {Object} req - Express request object
     * @param {Object} res - Express response object
     */
    async healthCheck(req, res) {
        try {
            ...
            logger.error('PayPal webhook health check failed', {
                error: error.message
            });
    ...
    /**
     * Middleware to validate PayPal webhook headers
     * @param {Object} req - Express request object
     * @param {Object} res - Express response object
     * @param {Function} next - Next middleware function
     */
    validatePayPalHeaders(req, res, next) {
        ...
        if (missingHeaders.length > 0) {
            ...
            return res.status(400).json({
                ...
                message: 'Required PayPal webhook headers are missing',
                missingHeaders
            });
        }
     ...
     * PayPal requires the raw body for signature verification
     * @param {Object} req - Express request object
     * @param {Object} res - Express response object
     * @param {Function} next - Next middleware function
     */
...
module.exports = {
    handleWebhook: paypalController.handleWebhook.bind(paypalController),
    getStatistics: paypalController.getStatistics.bind(paypalController),
    retryFailedWebhooks: paypalController.retryFailedWebhooks.bind(paypalController),
    healthCheck: paypalController.healthCheck.bind(paypalController),
    validatePayPalHeaders: paypalController.validatePayPalHeaders.bind(paypalController),
    parseRawBody: paypalController.parseRawBody.bind(paypalController)
};
