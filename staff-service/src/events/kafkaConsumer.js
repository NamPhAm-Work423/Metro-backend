...
/**
 * Handle user.created events for staff
...
async function handleUserCreatedEvent(payload) {
    try {
        ...
        // Only process staff role
        if (!payload.roles || !payload.roles.includes('staff')) {
            console.log('Ignored user.created without staff role', { userId: payload.userId });
            return;
        }

        // TODO: Call staff service to create staff profile
        console.log('Staff profile creation would happen here', { userId: payload.userId });
...
async function start() {
    try {
        ...
        console.log('Staff service Kafka consumer connected');
        ...
        console.log('Staff service Kafka consumer is running');
    } catch (error) {
...
async function stop() {
    try {
        ...
        console.log('Staff service Kafka consumer disconnected');
    } catch (error) {
